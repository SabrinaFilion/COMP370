...
class NewsAPITestCase(unittest.TestCase):
    ...
    #Testing that our function raises an exception if we do not provide keywords
    def test_fail_nokeywords(self):
        with self.assertRaises(TypeError):
            fetch_latest_news(api_key,[],lookback_days=10)
            

    #Testing that our function respects the lookback days
    def test_lookback_days_respected(self):
        data=fetch_latest_news(api_key,["health"],lookback_days=5)
    ...
    def test_fail_nonalpha_keyword(self):
        with self.assertRaises(ValueError):
            fetch_latest_news(api_key,"money$",lookback_days=10)
